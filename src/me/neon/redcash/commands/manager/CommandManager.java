package me.neon.redcash.commands.manager;
...
public class CommandManager implements CommandExecutor, TabCompleter {
	...
	private String message_noConsole = "§cVocê deve ser um jogador para utilizar este comando";
	private String message_noPerms = "§cVocê não tem permissão para fazer isso.";
	private String message_noCommand = "§7O comando que você digitou não existe ou está escrito incorretamente.";
	private List<String> message_syntaxError = Arrays.asList(
			"§cSintaxe inválida.",
			"§7A sintaxe inválida é: §c" + "%syntax%" + "§7."
			);
	...
	public SimpleNestedCommand registerCommandDynamically(AbstractCommand abstractCommand) {
        ...
        abstractCommand.getCommands().stream().forEach(cmd -> {
            ...
            if (pcmd != null) {
                ...
                plugin.getLogger().warning("Falha ao registrar o comando: /" + cmd);
            }
        });
        return nested;
    }
    ...
    public CommandManager setExecutor(String command) {
        ...
        if (pcmd != null) {
            ...
            plugin.getLogger().warning("Falha ao registrar o comando: /" + command);
        }
        return this;
    }
