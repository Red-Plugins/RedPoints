...
public class SimpleNestedCommand {
	...
	final LinkedHashMap<String, AbstractCommand> children = new LinkedHashMap<String, AbstractCommand>();
	...
	public SimpleNestedCommand addSubCommands(AbstractCommand[] commands) {
		Stream.of(commands).forEach(
				command -> command.getCommands().stream().forEach(cmd -> children.put(cmd.toLowerCase(), command)));
		return this;
	}
}
