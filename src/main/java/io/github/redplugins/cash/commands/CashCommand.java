package io.github.redplugins.cash.commands;
...
import io.github.redplugins.cash.manager.CashManager;
import io.github.redplugins.cash.manager.Messages;
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length >= 1) {
			if (args[0].equalsIgnoreCase("set")) {
				if (player.hasPermission(Configuration.config.getYaml().getString("PermissionCommand"))) {
			...
			if (args.length == 1) {
				...
				if (args[0].equalsIgnoreCase("help") || (args[0].equalsIgnoreCase("ajuda"))) {
					for (String message : Configuration.messages.getYaml().getStringList("CashHelpMessage")) {
